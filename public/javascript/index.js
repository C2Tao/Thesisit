...
treeJSON = d3.json("exJSON/graphTUI.json", function(error, treeData) {
    ...
    function initiateDrag(d, domNode) {
        ...
        //the red nodes covers undragging nodes
        ...
        // if dragging node has children in nodes, remove the links and nodes
        //console.log(nodes);
        if (nodes.length > 1) { //include dragging node
            // remove dragging node's link paths to child nodes
            ...
            nodesExit = svgGroup.selectAll("g.node")
                                .data(nodes, 
                                    function(d) { //this function's purpose remain unknown
                                        //times++;
                                        //console.log(d.name);
                                        return d.id;
                                    }
        ...
        // remove dragging node's parent link
        parentLink = tree.links(tree.nodes(draggingNode.parent));
        
        svgGroup.selectAll('path.link').filter(function(d, i) {
            if (d.target.id == draggingNode.id) {
                return true; //would be selected
            }
            return false; //get rid of
        }).remove();
